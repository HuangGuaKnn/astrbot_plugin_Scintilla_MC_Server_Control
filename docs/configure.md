...
插件全部配置共 **10 个分组、86 项**。多数情况下只需要填「连接与服务端」里的 RCON 信息，其余保持默认即可使用。
...
> [!TIP]
> 下面四项（超时 / 结束判定 / 哨兵探测 / 静默窗口）在 WebUI 设置页收在「RCON 连接」卡片底部的
> **「进阶设置」折叠区**里，默认收起、点标题展开 —— 卡片默认只显示地址、端口、密码三样必填项。
> 折叠区标题旁会显示当前值（如 `当前：超时 5s · sentinel（结束哨兵）`），收起时也看得见配置状态。
...
**`rcon_end_mode`** — 响应结束判定方式  
<sub>文本 · 默认：sentinel</sub>

可选值：`sentinel` / `idle`

> [!IMPORTANT]
> RCON 协议本身**没有**「响应到此结束」的标记，靠「静默多久算读完」永远只是猜。
> `sentinel`（默认）在发完命令后紧跟一条无害探测命令，用它的响应作为结束边界 —— 慢分包、半包停顿都不会被误判成「读完了」。
> 只有服务端完全不回应探测命令时才需要改成 `idle`（靠静默窗口猜，服务端分包间隔偏大时长响应可能被截断）。

**`rcon_probe_command`** — 结束哨兵用的探测命令  
<sub>文本 · 默认：空</sub>

> 留空即空命令：服务端照样会回一个响应包，而且它本身没有任何副作用。若你的服务端对空命令报错刷屏，可填一条只读命令（例如 `list`）。

**`rcon_idle_probe`** — 静默窗口（秒，仅 `idle` 生效）  
<sub>小数 · 默认：0.5</sub>

> 收到响应包后再等这么久没有新包才认为读完。0.05 ~ 5。`sentinel` 模式下本项不生效。

> [!NOTE]
> 连续多次无法确认响应边界时，插件会**自动降级**为 `idle` 并在 WebUI 设置页常驻显示「已自动降级」；点那一行的「重建 RCON 连接」即可回到配置的方式。
...
> [!IMPORTANT]
> 只管「命令工具」：mc_execute_command（执行指令）、mc_give_item（发物品）、mc_broadcast（广播）；喊话、状态、查询、绑定这类插件自带功能不受本策略影响（想限制喊话请改 say_command_public）。whitelist=白名单（默认）：非管理员完全不能使用口头命令工具；blacklist=黑名单：所有玩家都能使用命令工具，但危险命令（stop/op/ban/kick/whitelist…）与权限等级 3/4 的服务器管理命令仍仅管理员可用

**`permission_hint_mode`** — 权限前置提醒触发时机  
<sub>枚举 · 默认：`on_demand`</sub>

可选值：`always` / `on_demand` / `off`

> 控制何时把权限说明追加到本轮用户消息里（**不改写系统提示词**，避免动态内容破坏提示词缓存命中）：
> **`always`** —— 每次对话都注入（旧版行为）；
> **`on_demand`（默认）** —— 只在本轮消息涉及 MC（命中话题关键词）或本会话近期试图调用过 MC 工具时才注入，日常闲聊零打扰；
> **`off`** —— 从不注入，只在调用被拦时才告知。
> 三档都**不影响权限闸门与会话闩锁** —— 注入只是「省一次往返」的优化，真正的安全边界始终在工具层。
> 改完点「保存全部设置」即时生效，无需重载插件；填非法值时自动回落 `on_demand`。

**`permission_hint_keywords`** — MC 话题关键词（按需注入用）  
<sub>列表 · 默认：内置 29 词</sub>

> 只有本轮消息**命中其中之一**（或本会话近期调用过 MC 工具）时才注入权限前置提醒，日常闲聊因此零打扰。默认值就是内置词表，可在设置页直接增删，或追加自己服务器里的说法（如「开黑」「刷怪塔」「腐竹」）。留空 = 回退内置词表；匹配不区分大小写、按子串命中；仅在「触发时机 = `on_demand`」时生效。

**`permission_hint_injection`** — 权限前置提醒（旧开关·兼容保留）  
<sub>开关 · 默认：开</sub>

> 兼容保留：`false` 等价于「触发时机 = `off`」。新配置请改用 `permission_hint_mode` —— 旧版写死的 `true` 现在会落到默认的 `on_demand`（日常闲聊不再注入），想恢复「每轮都提醒」请把 `permission_hint_mode` 设为 `always`。
...
**`knowledge_search_engine`** — 知识库检索引擎  
<sub>枚举 · 默认：`bm25`</sub>

> **`bm25`（推荐）**：中文二元切分 + BM25 + 倒排索引。按字序匹配（「黄铜」≠「铜黄」、查 `tac` 不会捞出 `taconite`），
> 权重自动压低「通用 / 方案 / 规则」这类高频套话。实测：26 条语料 17 组查询下结果精确率 20.5% → 58.6%（2.9 倍）、
> 无关条目减少 82%；5000 条规模单次检索 6.3ms → 2.0ms。
> **`legacy`（旧版兼容）**：中文按单字、英文按整段做子串命中，字面与历史检索结果完全一致，但容易连锁误召回
> （查「满配一把枪」会捞出一堆含「配 / 方 / 案」的条目）。仅在需要与历史结果完全对齐时才选。
> 改完点「保存全部设置」即时生效，无需重载插件；填非法值时自动回落 `bm25`。
